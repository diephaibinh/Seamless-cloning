...
for dirpath, dirnames, fnames in subfolders:
    ...
    naive = [poisson.preview(source_img[:,:,i], target_img[:,:,i], mask) for i in range(channels)]
    # Merge the channels back into one image
    result = cv2.merge(result_stack)
    result_naive = cv2.merge(naive)
    ...
    cv2.imwrite(path.join(output_dir, 'result_naive.png'), result_naive)
